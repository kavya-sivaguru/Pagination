...
var state = {
    ...
    'rows': 10, //no. of rows in the table
    'pgBtn': 10, //no. of pagination buttons apart from "previous" & "next"
}

buildTable();

function pagination(table_data, currentPage, rows) {
    var trimstartPoint = (currentPage - 1) * rows;
    var trimendPoint = trimstartPoint + rows;

    var trimData = table_data.slice(trimstartPoint, trimendPoint);
...
function buildTable() {
    ...
    var item = pagination(state.table_data, state.currentPage, state.rows);
...
function pageButtons(pages) {
    ...
    var maxLeft = (state.currentPage - Math.floor(state.pgBtn / 2));
    var maxRight = (state.currentPage + Math.floor(state.pgBtn / 2));

    if (maxLeft < 1) {
        maxLeft = 1;
        maxRight = state.pgBtn;
    }
    if (maxRight > pages) {
        maxLeft = pages - (state.pgBtn - 1);
        maxRight = pages;
        if (maxLeft < 1) {
            maxLeft = 1;
        }
    ...
    var pag_button_first = createElemet('button', 'btn btn-light list-group-item');
    pag_button_first.setAttribute('onclick', `index_pre(${state.currentPage})`);
    pag_button_first.innerHTML = 'Previous';
    pag_button_first.style.color = 'teal';
    pag_button_first.style.fontWeight = "bolder";
    var pag_button_last = createElemet('button', 'btn btn-light list-group-item');
    pag_button_last.setAttribute('onclick', `index_next(${state.currentPage},${pages})`);
    pag_button_last.innerHTML = 'Next';
    pag_button_last.style.color = 'teal';
    pag_button_last.style.fontWeight = "bolder";


    for (var currentPage = maxLeft; currentPage <= maxRight; currentPage++) {
        var pag_button = createElemet('button', 'btn btn-sm btn-light list-group-item');
        pag_button.value = currentPage;
        pag_button.style.color = 'orange';
        pag_button.style.fontWeight = 'bolder';
        pag_button.setAttribute('onclick', 'pagechange(this.value)');
        pag_button.innerHTML = currentPage;
...
function createTable(arr) {
    var table = createElemet('table', 'mt-3 table text-center', 'table_content');
    var thead = createElemet('thead', 'bg-warning');
